...
const managerArkham = async ({ prisma }: Props): Promise<any> => {
  ...
  do {
    try {
      ...
      const users: { id: number; address: string }[] =
      ...
      const rawTxs = (await Promise.all(
        users.map((user) =>
          fetch(
            `https://api.arkhamintelligence.com/intelligence/address/${user.address}/all`,
            {
              headers: {
      ...
      console.log(
        `Row grouping completed, ${data.length} groups found. Creating data chunks`,
      );
