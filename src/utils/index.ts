...
export const getApplicationTx = async ({
  chainId,
  projectId,
  block,
  roundId,
...
export const createExtraParams = (next_page_params: NextPageParams) => {
  return Object.keys(next_page_params).reduce(
  ...
  return (
    (
      await ankrProvider.getLogs({
        ...
        topics: [
          [
            "0xdc7180ca4affc84269428ed20ef950e745126f11691b010c4a7d49458421008f",
          ],
        ],
...
export const fetchRoundDistributionData = async ({
  ...
  try {
    ...
    const [{ result: metaData }, { result: tokenAddress }] =
      ...
      if (metaPtr && metaPtr.length > 0) {
        ...
        if (log && tokenAddress) {
          ...
          if (nTokenAddress !== ethers.constants.AddressZero) {
            const [{ result: decimal }, { result: code }] =
              await client.multicall({
                contracts: [
                  {
                    ...tokenContractConfig,
